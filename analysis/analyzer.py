...
genai.configure(api_key="AIzaSyCSVJp6EKxlVFw0tt8f0B-GIBrlLGj63Js")
import joblib
...
class SecurityAnalyzer:
    def __init__(self):
        ...
        self.ml_model = joblib.load(os.path.join(os.path.dirname(__file__), 'failles.pkl'))


    def predict_vulnerability(self, metrics):
        expected_features = [
            "complexite_cyclomatique",
            "nombre_branches",
            "profondeur_heritage",
            "nombre_classes",
            "longueur_moyenne_fonctions",
            "parametres_moyens_par_fonction",
        ...
        filtered_metrics = {k: metrics.get(k, 0) for k in expected_features}
        df = pd.DataFrame([filtered_metrics])[expected_features]
        try:
            prediction = self.ml_model.predict(df)[0]
            return "Défaut logiciel détecté" if prediction == 1 else "Aucun défaut logiciel détecté"
    ...
    def analyze_project(self, project_path):
        ...
        metrics["ml_prediction"] = self.predict_vulnerability(metrics)
        self.report["summary_metrics"] = metrics

        # Génération des recommandations Gemini
        try:
            issues_summary = self.summarize_issues()
            metrics_summary = self.summarize_metrics()
            gemini_recos = get_gemini_recommendations(issues_summary, metrics_summary)
            if gemini_recos:
                self.report["llm_recommendations"] = gemini_recos
            else:
                self.report["llm_recommendations"] = None
        except Exception as e:
            self.report["llm_recommendations"] = None
            self.report["llm_error"] = str(e)
    ...
    def summarize_metrics(self):
        """
        Résume les métriques et recommandations associées pour Gemini.
        """
        metrics = self.report.get("summary_metrics", {})
        recos = metrics.get("recommendations", [])
        lines = []
        for reco in recos:
            metric = reco.get("metric", "")
            severity = reco.get("severity", "").upper()
            recommendation = reco.get("recommendation", "")
            value = reco.get("value", "")
            lines.append(f"{metric} ({severity}) : {recommendation}\n[Valeur actuelle : {value}]")
        return "\n".join(lines) if lines else "Aucune métrique critique à signaler."
    


    def summarize_issues(self):
        """
        Résume les failles détectées pour les envoyer à Gemini.
        """
        summary = []
        for filepath, issues in self.report["detailed_report"].items():
            for issue in issues:
                summary.append(
                    f"- Fichier : {os.path.basename(filepath)}, ligne {issue.get('line', '?')}, "
                    f"type : {issue.get('type', '?')}, gravité : {issue.get('severity', '?')}, "
                    f"message : {issue.get('message', '')}"
                )
        return "\n".join(summary) if summary else "Aucune faille critique détectée."
...
def get_gemini_recommendations(issues_summary, metrics_summary):
    """
    Utilise Gemini pour générer des recommandations personnalisées à partir d'un résumé des failles ET des métriques.
    """
    prompt = (
        "Voici un résumé des failles de sécurité et de qualité détectées dans un projet Python :\n"
        f"{issues_summary}\n\n"
        "Voici un résumé des métriques globales du projet et des recommandations associées :\n"
        f"{metrics_summary}\n\n"
        "Donne-moi des recommandations détaillées, concrètes et adaptées à ce contexte, en français, pour améliorer la sécurité ET la qualité du code par rapport à ces deux résumés. Structure la réponse en points clairs, en séparant bien les recommandations sur les failles et celles sur les métriques."
    )
    try:
