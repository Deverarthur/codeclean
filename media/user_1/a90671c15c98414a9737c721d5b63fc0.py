...
def modifier_travail(request, travail_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            encadrements = Encadrer.objects.filter(travail=travail)
            for encadrement in encadrements:
                if travail.typeTravail in ['TFC', 'Mémoire'] or (travail.typeTravail == 'Projet Ordinaire' and encadrement.role == 'Rapporteur'):
                    encadrement.decision = None
                    encadrement.motif = None
                    encadrement.save()
...
def details_travail_faculte_bibliothecaires(request, travail_id):
    travail = get_object_or_404(Travail, id=travail_id)
    emprunt = Emprunter.objects.filter(travail=travail, etat="Emprunté").first()  # Récupère l'emprunt en cours
    directeur = Encadrer.objects.filter(travail=travail, role='Directeur').first()
    rapporteur = Encadrer.objects.filter(travail=travail, role='Rapporteur').first()
